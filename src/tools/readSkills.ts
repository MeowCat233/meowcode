...
// 1. 参数类型定义
export interface ReadSkillsArgs {
    skillName?: string; // 可选。如果传入，则读取具体某个技能文件的内容；不传就列出所有可用技能
}
 ...
 */
function validateSkillFormat(content: string): { valid: boolean; missing?: string } {
...
// 4. 工具核心逻辑实现
export function readSkills(args: ReadSkillsArgs): string {
    try {
