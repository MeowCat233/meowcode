import { ChatMessage, summarize } from "../ds.js";
...
export class MemoryManager {
  ...
  // 严格遵循官方类型的标准历史容器
  private uncompressedHistory: ChatMessage[] = [];
  private maxBufferSize: number;
  private retainCount: number;
  private compressionPrompt: string;
...
请直接输出更新后的摘要。`;
   ...
   */
  public append(roleOrObj: any, content?: any) {
  ...
  public async compress(): Promise<string> {
    if (!this.shouldCompress()) return this.currentSummary;
    ...
    const decoratedMessages: ChatMessage[] = [
      {
        role: 'system',
